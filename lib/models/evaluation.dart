...
///
/// In an 8-team bracket: slots in the same R1 match → 1, same quarter → 2,
/// same half → 3, etc.
...
///
/// Iteratively finds the first offending pair and tries every possible swap
/// that strictly reduces the total number of too-early meetings.
...
void mapTablesDynamic(Knockouts knock, {int tableCount = 6}) {
  ...
  for (int i = 0; i < knock.superCup.matches.length; i++) {
    knock.superCup.matches[i].tischNr = (5 + i) % tableCount + 1;
  }
}
...
Knockouts evaluateGroups(Tabellen tabellen) {
  ...
  // Ordered by group rank (demoted 2nds → remaining 3rds → all 4ths) so
  // that 4th-place teams always sink to the lowest league.  Within each
  // rank tier teams are sorted by cross-group performance.
  // Europa is filled first with a full power-of-2 bracket, Conference gets
  // the rest.  If Conference would have < 2 teams those teams are excluded.
  final rest = <String>[];
